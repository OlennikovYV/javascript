var encryptThis = function(text) {
    const encodeWord = word => {
        const charOne = word.charCodeAt(0);
        const length = word.length;
        if (length < 3) {
            return charOne + word.slice(1);
        } else {
            const charTwo = word.slice(-1);
            const charLast = word[1];
            return charOne + charTwo + word.slice(2, -1) + charLast;
        }
    }

    return text
        .split(' ')
        .map(word => encodeWord(word))
        .join(' ');
}
